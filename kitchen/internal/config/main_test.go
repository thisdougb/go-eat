...
func TestString(t *testing.T) {
	...
	assert.Equal(t, "AAA", String("_TEST_STR_VALUE"), "no env var set")

	// test: now override the defaultValue[map] using an env var value
	os.Setenv("_TEST_STR_VALUE", "hello")
	assert.Equal(t, "hello", String("_TEST_STR_VALUE"), "env var set")
	os.Unsetenv("_TEST_STR_VALUE")
}
...
func TestInt(t *testing.T) {
	...
	assert.Equal(t, 10, Int("_TEST_INT_VALUE"), "no env var set")

	// test: now override the defaultValue[map] using an env var value
	os.Setenv("_TEST_INT_VALUE", "20")
	assert.Equal(t, 20, Int("_TEST_INT_VALUE"), "env var set")
	os.Unsetenv("_TEST_INT_VALUE")

	// test: now we use a non-int env var, which should be ignored
	os.Setenv("_TEST_INT_VALUE", ";")
	assert.Equal(t, 10, Int("_TEST_INT_VALUE"), "env var not int")
	os.Unsetenv("_TEST_INT_VALUE")
...
func TestBool(t *testing.T) {
	...
	assert.Equal(t, false, Bool("_TEST_BOOL_VALUE"), "no env var set")

	// test: now override the defaultValue[map] using an env var value
	os.Setenv("_TEST_BOOL_VALUE", "true")
	assert.Equal(t, true, Bool("_TEST_BOOL_VALUE"), "env var set")
	os.Unsetenv("_TEST_BOOL_VALUE")

	// test: now we use a non-int env var, which should be ignored
	os.Setenv("_TEST_BOOL_VALUE", "hello")
	assert.Equal(t, false, Bool("_TEST_BOOL_VALUE"), "env var not int")
	os.Unsetenv("_TEST_BOOL_VALUE")
}
